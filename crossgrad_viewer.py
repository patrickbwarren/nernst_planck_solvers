...
sigma = np.stack([sigma, sigma]) # double up for vector operations 
...
spc = del2(phi)[1:-2, 1:-2] # space charge (a.u)

phimin, phimax = np.min(phi), np.max(phi)
print('minmax: phi = ', phimin, phimax)
phi_levels = list(np.linspace(0.0, 70.0, 10))
...
curl_levels = list(np.linspace(-1.0, 5.0, 7))

rmin, rmax = np.min(rhotot), np.max(rhotot)
print('minmax: rhotot = ', rmin, rmax)
r_levels = list(np.linspace(0.0, 4.0, 11))
...
if args.drift:
    ...
    plt.subplot(2, 3, 2)
    plt.contour(x, y, phi, phi_levels, colors='black')
    plt.imshow(spc, extent=[1.0, Lx-1.0, 1.0, Ly-1.0], origin='lower', cmap='RdBu',
               vmin=-spclim, vmax=spclim, alpha=0.5)
    plt.title('phi and del2 phi')
    ...
    plt.subplot(2, 3, 4)
    plt.contour(x, y, sigma[0, :, :], sig_levels, colors='black')
    plt.imshow(sigma[0, :, :], extent=[0.0, Lx, 0.0, Ly], origin='lower', cmap='viridis',
